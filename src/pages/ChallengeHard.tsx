...
const ChallengeHard = () => {
  ...
  useEffect(() => {
    document.title = "Challenge Hard - Brick Breaker";
    loadLevelData();
  }, []);

  const loadLevelData = () => {
    const saved = localStorage.getItem('challengeHardProgress');
    const data = saved ? JSON.parse(saved) : {};
    const initialData: { [key: string]: LevelData } = {};
    for (let i = 1; i <= hardChallengeLevels.length; i++) {
  ...
  const handleLevelClick = useCallback((level: number, event?: React.MouseEvent | React.TouchEvent) => {
    ...
    if (levelData[levelKey]?.unlocked) {
      ...
      setTimeout(() => {
        navigate(`/game?mode=challenge&difficulty=hard&level=${level}`);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-red-200 to-pink-400 py-8 px-4" ref={containerRef}>
      <Button
        onClick={() => navigate('/challenge')}
        className="fixed top-2 left-2 bg-red-500 hover:bg-red-600 text-white rounded-full shadow-lg z-50"
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-6 sm:mb-8">
          <h1 className="text-3xl sm:text-5xl font-bold text-red-900 mb-2 sm:mb-4 drop-shadow-lg">
            CHALLENGE - HARD
          </h1>
          <p className="text-red-900/80 text-base sm:text-lg">
            Choose your challenge • {hardChallengeLevels.length} levels to conquer
          </p>
        </div>
        <div className="grid grid-cols-3 md:grid-cols-5 lg:grid-cols-10 gap-2 sm:gap-4 max-w-full sm:max-w-4xl mx-auto">
          {Array.from({ length: hardChallengeLevels.length }, (_, i) => {
            const level = i + 1;
            const levelKey = `level_${level}`;
            ...
            return (
              <button
                ...
                className={`
                  ...
                  text-red-900 font-bold text-xs xs:text-sm sm:text-base md:text-lg
                  transition-all duration-200 transform
                  ${isUnlocked 
                    ? `bg-gradient-to-br from-red-400 to-pink-500 
                       ${isTouched ? 'scale-95 from-red-500 to-pink-600' : 'hover:scale-110 hover:from-red-500 hover:to-pink-600'} 
              ...
              >
                ...
                <div className="flex flex-col items-center gap-1 w-full">
                  ...
                  {data && data.score > 0 && (
                    <span className="text-xs sm:text-sm md:text-base lg:text-xs xl:text-xs text-red-900/70">
                      {data.score}
                    </span>
                  )}
                </div>
        ...
        <div className="text-center mt-6 sm:mt-8 text-red-900/60">
